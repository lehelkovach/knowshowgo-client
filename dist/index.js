...
var KnowShowGoClient = class {
   ...
   */
  constructor({ baseUrl = "http://localhost:3000", fetchImpl, prototypeApiPrefix = "/api2.0", auto_connect = false } = {}) {
  ...
  _assertContractPath(method, path) {
    if (!this._enforceContract || !this._contract)
      return;
  ...
  get_assertions({ subject = null, predicate = null, obj = void 0 } = {}) {
    const params = {};
  ...
  add_verified_fact({ subject, predicate, obj, sources = [] }) {
    ...
    const confidence = source?.trust_score ?? 1;
