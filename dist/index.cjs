...
__export(index_exports, {
  KnowShowGoClient: () => KnowShowGoClient
});
module.exports = __toCommonJS(index_exports);

// js/client.js
var KnowShowGoClient = class {
  /**
  ...
  async _request(method, endpoint, { json, params } = {}) {
    ...
    const hasJsonBody = json !== void 0;
    const res = await this.fetch(url.toString(), {
      method,
      headers: hasJsonBody ? { "content-type": "application/json", accept: "application/json" } : { accept: "application/json" },
      body: hasJsonBody ? JSON.stringify(json) : void 0
    });
    if (res.status === 204) return null;
    const contentType = res.headers.get("content-type") || "";
    const payload = contentType.includes("application/json") ? await res.json() : await res.text();
    if (!res.ok) {
      const message = typeof payload === "object" && payload && payload.error ? payload.error : `Request failed: ${method} ${endpoint} (${res.status})`;
  ...
  search_procedures(queryOrPayload, { topK } = {}) {
    const payload = typeof queryOrPayload === "object" && queryOrPayload !== null ? queryOrPayload : { query: queryOrPayload, topK };
  ...
  get_channel_feed(actorIdOrParams) {
    const params = typeof actorIdOrParams === "object" && actorIdOrParams !== null ? actorIdOrParams : { actorId: actorIdOrParams };
  ...
  // ===== Associations =====
  add_association({ from_concept_uuid, to_concept_uuid, relation_type, strength = 1 }) {
